...
#include "winhandle.h"														// Header File For The NeHeGL Basecode
...
/*int WINAPI WinMain_new (HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	window.keys					= &keys;								// Window Key Structure
	window.init.application		= &application;							// Window Application
	window.init.title			= "Alex's Stereo Camera";			// Window Title
	window.init.width			= 800;									// Window Width
	window.init.height			= 600;									// Window Height
	window.init.bitsPerPixel	= 16;									// Bits Per Pixel
	window.init.isFullScreen	= TRUE;									// Fullscreen? (Set To TRUE)
	...
	//StereoDisplay my_stereo(&window, &keys,window.init.width, window.init.height );
	//Initialize (&window, &keys,window.init.width, window.init.height );
	while (g_isProgramLooping)											// Loop Until WM_QUIT Is Received
	{
        g_isProgramLooping = FALSE;											// Program Looping Is Set To FALSE
		// Create A Window
				...
				while (isMessagePumpActive == TRUE)						// While The Message Pump Is Active
				{
					...
					else												// If There Are No Messages
					{
						...
						else											// If Window Is Visible
						{
							...
							//   break; // End while
							if( FALSE == Update(tickCount-window.lastTickCount) )     	// Update The Counter
							   break; // End while
							window.lastTickCount = tickCount;			// Set Last Count To Current Count
							Draw();
							//my_stereo.DrawTexture ();
